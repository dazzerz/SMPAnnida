...
# 1. Definisikan Data secara granular (per line/agenda)
summary_data = [
    ["RINGKASAN TIMELINE PERSIAPAN & EKSEKUSI PPDB", "", "", "", ""],
    ["", "", "", "", ""],
    ["Fase", "Periode", "Fokus Utama", "Progress", "Detail Tab"],
    ["Week 1", "29 - 30 Aug 2026", "Strategi Pemasaran & Perencanaan Promosi", "FALSE", "Lihat Sheet 'W1 (29-30 Aug)'"],
    ["Week 2", "5 - 6 Sep 2026", "Regulasi, Persyaratan & SOP Alur Pendaftaran", "FALSE", "Lihat Sheet 'W2 (5-6 Sep)'"],
    ["Week 3", "12 - 13 Sep 2026", "Penyusunan Instrumen & Mekanisme Seleksi", "FALSE", "Lihat Sheet 'W3 (12-13 Sep)'"],
    ["Week 4", "19 - 20 Sep 2026", "Finalisasi Kelulusan & Kesiapan Sistem", "FALSE", "Lihat Sheet 'W4 (19-20 Sep)'"],
    ["Week 5", "26 - 27 Sep 2026", "Pleno Final, Mitigasi Risiko & Isu Tambahan", "FALSE", "Lihat Sheet 'W5 (26-27 Sep)'"],
    ["Eksekusi", "28 Okt - 4 Nov 2026", "Kunjungan Lapangan & Distribusi Materi Promosi", "FALSE", "Lihat Sheet 'Eksekusi Lapangan'"],
]
...
w1_data = [
    ...
    ["30-Aug-2026", "Minggu", "Finalisasi Konsep & Copywriting Materi Promosi (Brosur/Spanduk)", "Desain Siap Cetak", "Tim Promosi", "FALSE"],
...
# 2. Re-create all worksheets to start fresh cleanly
print("Menghapus sheet lama dan membuat yang baru...")
old_worksheets = sh.worksheets()

new_worksheets = {}
for title, data in sheets_config.items():
    ws = sh.add_worksheet(title=title + "_new", rows=30, cols=8)
    new_worksheets[title] = ws
    time.sleep(1)

# Hapus semua yang lama (kecuali tidak bisa menghapus sheet terakhir, tapi karena sudah ada yang baru, aman)
...
# Rename yang baru jadi benar
for title, ws in new_worksheets.items():
    ws.update_title(title)
    time.sleep(1)

# 3. Masukkan Data dan Desain menggunakan Batch Update API untuk super fast & reliable design
print("Mengaplikasikan desain profesional & Checkbox...")
batch_requests = []
...
for title, ws in new_worksheets.items():
    ...
    # Title merge and format (Row 1 & 2)
    ...
    # Body Wrap text & Vertical Align Top
    batch_requests.append({
        "repeatCell": {
            "range": {"sheetId": ws_id, "startRowIndex": 3, "endRowIndex": len(data), "startColumnIndex": 0, "endColumnIndex": cols},
            "cell": {
                "userEnteredFormat": {
                    "wrapStrategy": "WRAP",
    ...
    # Checkbox Validation (Kolom Status)
    # Tentukan index kolom Status
    status_col_idx = 3 if is_summary else (5 if is_kunj else 5)
    
    batch_requests.append({
        "setDataValidation": {
            "range": {"sheetId": ws_id, "startRowIndex": 3, "endRowIndex": len(data), "startColumnIndex": status_col_idx, "endColumnIndex": status_col_idx+1},
            "rule": {
    ...
    # Kolom Lebar (Column Widths)
    if is_summary:
        widths = [80, 150, 350, 80, 200]
    elif is_kunj:
        widths = [100, 80, 180, 300, 120, 80, 200]
    else:
        # Tgl, Hari, Agenda, Output, PIC, Status
        widths = [100, 80, 350, 250, 150, 80]
...
print("SELESAI! Desain profesional telah diterapkan.")
