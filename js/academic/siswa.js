...
let isSiswaInitialized = false;

function initSiswaSection() {
    const siswaSection = document.getElementById('data-siswa');
    if (!siswaSection || isSiswaInitialized) return;
    isSiswaInitialized = true;
...
// Listen for both immediate DOM, hashchange, and dynamic sectionLoaded
document.addEventListener('DOMContentLoaded', initSiswaSection);
window.addEventListener('sectionLoaded', (e) => {
    if (e.detail && e.detail.id === 'data-siswa') {
        initSiswaSection();
    }
});
if (document.getElementById('data-siswa')) {
    initSiswaSection();
}

window.loadStudents = initSiswaSection;
export { initSiswaSection };
