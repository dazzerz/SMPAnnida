...
export async function openTeacherViewer(url, title, subtitle, type) {
        ...
        openExternal.onclick = async (e) => {
            ...
            if (isHtml) {
                try {
                    ...
                    console.warn('Open external tab error:', err);
                }
    ...
    if (isImage) {
        ...
        if (isDrive) {
            iframe.srcdoc = '<div style="color:#e2e8f0;background:#090d16;min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;font-family:system-ui,sans-serif;text-align:center;padding:2rem;"><div style="font-size:2.5rem;margin-bottom:1rem;">🌐</div><h3 style="margin:0 0 0.5rem;font-size:1.15rem;font-weight:700;color:#34d399;">Memuat Modul Interaktif...</h3><p style="margin:0;font-size:0.85rem;color:#94a3b8;">Sedang mengambil simulasi HTML dari Google Drive</p></div>';
