...
export async function deleteBudget(id) {
  const { data, error } = await supabaseClient.from('budgets').delete().eq('id', id).select();
  if (error) throw error;
  if (!data || data.length === 0) {
    throw new Error('Gagal menghapus budget. Kemungkinan akses ditolak (RLS) atau data sudah tidak ada.');
  }
}
