...
document.querySelector('.summary-card:nth-child(2)').style.display = 'none';
}
}
...
<td style="padding:0.5rem; text-align:center;">${rowCount++}</td>
<td style="padding:0.5rem;">${comp.name}</td>
<td style="padding:0.5rem; text-align:right;">${formatRp(item.rate)}</td>
...
<td style="padding:0.5rem; text-align:right;" class="edit-slip-subtotal" data-itemid="${item.id}">${formatRp(item.subtotal)}</td>
</tr>
`;
}
});
